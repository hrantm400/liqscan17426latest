...
// Local Candle / Signal types are intentionally LOOSE — the canonical
// types in `types/index.ts` are stricter (e.g. openTime: string only,
// price: number only) and would reject perfectly valid props the older
// chart consumers pass in. Keeping these here lets every existing
// caller pass through without coercion.
interface Candle {
  id?: string;
  symbol: string;
  timeframe: string;
...
/**
 * Full-page interactive signal chart, built on klinecharts.
 *
 * Renders:
 *   - 300-bar candle history with theme-aware colors
 ...
 *     CisdLwChartApi shim (utils/klineCisdAdapter — kept post-migration
 *     to avoid rewriting the working CISD path natively for klinecharts)
 *   - RSI sub-pane + divergence trend lines for RSI-divergence signals
 *   - TradingView iframe toggle
 *   - Theme switch via setStyles in place; resize via ResizeObserver;
 *     onPriceUpdate / onCandleUpdate notifications to the parent
 */
  ...
  useEffect(() => {
    ...
    if (isCisdFamilySignal(signal)) {
      ...
      const candleBars = slicedCandles.map((c) => ({
        // CISD adapter (utils/klineCisdAdapter) reads `time` as unix
        // seconds — the field type is the legacy LW shape but the value
        // is just a number, so cast through unknown.
        time: Math.floor(new Date(c.openTime).getTime() / 1000) as unknown as number,
        open: c.open,
        high: c.high,
        low: c.low,
        close: c.close,
    ...
    if (isRsi && slicedCandles.length > 30) {
      ...
      if (divResult) {
        ...
        if (priceLineId) {
          ...
          console.warn(
            '[InteractiveLiveChart] price-pane divergence overlay creation returned null',
          );
        }
        ...
        if (rsiPaneIdRef.current) {
          ...
          if (rsiLineId) {
            ...
            console.warn(
              '[InteractiveLiveChart] RSI-pane divergence overlay creation returned null',
              { rsiPaneId: rsiPaneIdRef.current },
            );
          }
        } else if (typeof console !== 'undefined') {
          console.warn(
            '[InteractiveLiveChart] RSI pane id not yet captured — divergence on RSI pane skipped',
          );
        }

      } else if (typeof console !== 'undefined') {
        console.warn(
          '[InteractiveLiveChart] detectLastDivergence returned null',
    ...
    // have a signal candle index. klinecharts' built-in `priceLine`
    // overlay renders a full-width horizontal line at the price level
    // with the value label on the price axis automatically.
