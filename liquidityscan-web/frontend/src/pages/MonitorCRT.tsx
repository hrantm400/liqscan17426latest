...
export function MonitorCRT() {
    ...
    const hasCrtAccess = hasFeature('crt');
    const isTierPaid = hasFullProductAccess || hasCrtAccess;
    const isSymbolAllowed = useCallback((symbol: string) => isTierPaid || baseIsSymbolAllowed(symbol, 'crt'), [isTierPaid, baseIsSymbolAllowed]);
    ...
    return (
        <>
            ...
            >
                <div className="flex overflow-x-auto md:overflow-visible snap-x no-scrollbar gap-4 md:grid md:grid-cols-2 lg:grid-cols-4 pb-2 md:pb-0">
                    {/* 1H */}
                    <AnimatedCard
            ...
            {isLoading && (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-background-dark/50 backdrop-blur-sm">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-primary font-mono font-bold animate-pulse">Scanning CRT Signals...</p>
                    </div>
                </div>
            )}
                                                ...
                                                >
                                                    ...
                                                    <div className={`px-4 py-2 dark:bg-black/40 light:bg-green-50 dark:border-t-white/5 light:border-t-green-300 flex justify-between items-center z-10 relative ${isLocked ? 'blur-[5px] select-none pointer-events-none' : ''}`}>
                                                        <div className="flex items-center gap-1.5">
                                                            <span className="text-[10px] text-primary font-bold uppercase">CRT</span>
                                                        </div>
                                                        {!isLocked && <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
